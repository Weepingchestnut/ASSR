...
_C.MODEL.MIXED_PRECISION = False

_C.MODEL.ACT_MODE = 'relu'
_C.MODEL.PRE_TRAIN = None       # pre train
_C.MODEL.CKP_STRICT = True      # ckp strict
_C.MODEL.CKP_IGNORE = None      # ckp ignore
...
_C.MODEL.PLANES = 64    # mid channel
_C.MODEL.STOCHASTIC_DEPTH = False   # stochastic depth 随机深度
_C.MODEL.STOCHASTIC_DEPTH_PROB = None  # survival probability
_C.MODEL.ZERO_INIT_RESIDUAL = True  # zero init residual
_C.MODEL.MULT_FLAG = True   # multiple flag
_C.MODEL.EXPANSION = 1  # expansion 扩大，扩充
_C.MODEL.KERNEL_SIZE = 3
_C.MODEL.SHIFT_MEAN = True  # shift mean
_C.MODEL.DILATION = False   # dilation 空洞
_C.MODEL.SE_REDUCTION = 4   # SE reduction
_C.MODEL.SHORT_SKIP = True  # short skip
_C.MODEL.AFFINE_INIT_W = 0.1    # affine init W 仿射
_C.MODEL.DEFORM_CONV = False    # Deformable convolution 可变形卷积
...
_C.DATASET.DATA_DIR = '/home/lzk/workspace/rcan-it/datasets'
_C.DATASET.DEMO_DIR = '../test'
# _C.DATASET.DATA_TRAIN = ['DF2K']
_C.DATASET.DATA_TRAIN = ['DIV2K']
# _C.DATASET.DATA_VAL = ['DF2K']
_C.DATASET.DATA_VAL = ['DIV2K']
# _C.DATASET.DATA_TEST = ['DF2K', 'Set5', 'Set14C', 'B100', 'Urban100', 'Manga109']
_C.DATASET.DATA_TEST = ['DIV2K', 'Set5', 'Set14C', 'B100', 'Urban100', 'Manga109']
# _C.DATASET.DATA_RANGE = [[1, 3550], [3551, 3555]]
_C.DATASET.DATA_RANGE = [[1, 800], [801, 900]]
_C.DATASET.DATA_EXT = 'bin'  # 'bin', 'sep' or 'img'
_C.DATASET.DATA_SCALE = [4]
_C.DATASET.OUT_PATCH_SIZE = 192  # square patch size of model output when training
_C.DATASET.RGB_RANGE = 255
_C.DATASET.CHANNELS = 3

_C.DATASET.CHOP = False
_C.DATASET.CHOP_PAD = 10        # chop pad
_C.DATASET.CHOP_THRES = 40000   # chop thres

_C.DATASET.OVERLAP = CN({"ENABLED": False})     # overlap 重叠部分
_C.DATASET.OVERLAP.STRIDE = 48                  # overlap stride
_C.DATASET.OVERLAP.SIZE = 96                    # overlap size

_C.DATASET.REJECTION_SAMPLING = CN({"ENABLED": False})      # 是否对输入的HR、LR patch pair进行调整
...
# Augment
# -----------------------------------------------------------------------------
_C.AUGMENT = CN({"ENABLED": True})          # 数据增强，见 data.common.augment
_C.AUGMENT.CHANNEL_SHUFFLE = False          # channel shuffle
_C.AUGMENT.INVERT = False                   # invert 翻转
...
_C.SOLVER.ITERATION_SAVE = 20000    # iteration save
_C.SOLVER.ITERATION_TOTAL = 200000  # iteration total
_C.SOLVER.TEST_EVERY = 1000         # test every
...
_C.SOLVER.TAIL_ONLY_ITER = -1   # tail only iteration 仅训练尾部

_C.SOLVER.BASE_LR = 1e-4    # base learning rate
_C.SOLVER.BIAS_LR_FACTOR = 1.0      # bias lr factor
_C.SOLVER.MIN_LR = 0.0      # min lr
...
_C.SOLVER.SAMPLES_PER_BATCH = 8     # batch size
...
_C.SOLVER.TEST_ONLY = False     # test only
_C.SOLVER.GAN_K = 1
