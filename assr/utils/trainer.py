...
from assr.data.common import Mixup
from assr.utils import utility
from assr.utils.solver.build import build_optimizer, build_lr_scheduler, build_swa_model
...
class Trainer:
    def __init__(self, cfg, rank, loader, model, loss, device, ckp=None):
        ...
        if not cfg.SOLVER.TEST_ONLY:
            ...
            self.mixed_fp = self.cfg.MODEL.MIXED_PRECISION
            # gradient scaling for mixed-precision training 混合精度训练的梯度缩放
    ...
    def train(self):
        ...
        for i in range(self.iter_start, self.iteration_total):
            ...
            # full-precision when fine-tuning tail 全精度微调尾部
            ...
            self.scheduler_step(i)  # update SWA model if it exists 若SWA模型存在，更新该模型
            if (self.rank is None or self.rank == 0) and i % 10 == 0:
                self.log_train(i, loss, timer)

            del lr, hr, sr, loss    # Release some GPU memory 释放部分GPU显存
    ...
    def freeze_tail(self, defrost: bool = False):
        if defrost:
            print("Defrost all frozen layers.")     # 解冻所有冻结层
            for param in self.model.parameters():
                param.requires_grad = True
            return

        # keep only the tail trainable 保持仅有尾部可训练
            ...
            for idx_data, d in enumerate(self.loader_test):
                for idx_scale, scale in enumerate(self.scale):
                    ...
                    for lr, hr, filename in tqdm(d, ncols=80):
                        lr = lr.to(self.device, non_blocking=True)
                        with autocast(enabled=self.cfg.MODEL.MIXED_PRECISION):
                        ...
                        if self.cfg.LOG.SAVE_GT:    # 是否同时保存GT
